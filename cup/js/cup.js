var cup = {
  ...
  reload: function() {
  	var cupString = localStorage.getItem('cup', cupString);
  	if (! cupString) {
  		return; // Nothing found in local storage
  	}

  	// Merge together with history
  ...
  highlightHorse: function(horse) {
  	...
  	$('.field ul').find('li:contains(' + horse + ')').addClass('active');			 //sudo selector
  ...
  placeBet: function () {
    ...
    $('#gambler').val('').focus(); // Clear the input.
